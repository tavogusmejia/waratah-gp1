-- ============================================================================
-- GP1-MUR Material Register - baseline.
--
-- This is the schema as it was FIRST APPLIED BY HAND on 2026-09-07, captured
-- as a migration so that everything after it can be one.
--
-- It was written rather than produced by `supabase db pull` because db pull
-- runs pg_dump inside Docker, which is not installed here - and because the
-- exact SQL that was applied is known: it is the two files below, verbatim.
-- On the live database this migration is recorded as already applied
-- (`supabase migration repair`), so it never re-runs there. On a fresh
-- database it builds the schema from nothing, which is what a baseline is for.
--
-- Do not edit this file. Change the schema with a NEW migration:
